...
| 插件目录 | 名称 | 说明 |
|---|---|---|
| `plugins.v2/mediapipeline/` | 媒体入库流水线 | 四步合一：OpenList 扫描 + 网盘改名清洗(改 115 源防 insert 复活) + 增量整理刮削 + Emby 扫描；各步独立开关，含 115 防风控 + 预演。**现有生产主线** |
| `plugins.v2/sourcepipeline/` | 115 源流水线 | 新方向只读实现：来源实例与可复用 logic 分开配置，按请求预算广度优先遍历多层 `/115/...`，SQLite v3 断点续扫；再在缓存上产出垃圾清单与改名预演报告（规划不联网）。唯一网络 endpoint 是 `/api/fs/list`，无改名、删除或 STRM 生成能力 |
